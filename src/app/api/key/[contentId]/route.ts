...
const ACCESS_REGISTRY_ADDRESS = (process.env
  .NEXT_PUBLIC_ACCESS_REGISTRY_ADDRESS ||
  "0x41530D874f3DA5e7C998201BaA18aE0eC4A4fF2C") as `0x${string}`;
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Failed to grant access on-chain",
    };
  }
}
...
async function fetchMetadataFromIPFS(
  ...
  for (const gateway of IPFS_GATEWAYS) {
    try {
      ...
      console.warn(`Gateway ${gateway} failed for ${metadataCID}:`, error);
      continue;
    }
  }
  console.error(`All gateways failed for metadata: ${metadataCID}`);
  return null;
}
...
export async function GET(
  ...
  try {
    ...
    const consumerAddress = request.headers.get("x-consumer-address") as
      | `0x${string}`
      | null;
    ...
    if (paymentHeader) {
      ...
      const consumer = (settleResult.consumer ||
        consumerAddress) as `0x${string}`;
      const txHash = (settleResult.txHash ||
        "0x" + "0".repeat(64)) as `0x${string}`;

      if (consumer) {
        const grantResult = await grantAccessOnChain(
          contentId,
          consumer,
          txHash
        );
